...
# We specify the server parameters connect to the MCP tool server
...
async def main():
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            tools = await load_mcp_tools(session)          # load the tools 
            agent = create_react_agent(model, tools)       # create the agent
            ...
            while True:
                ...
                # user_input[:175000] => The goal is to get token error if the user input is too long.
